---
description: Create a well-structured git commit using the currently selected primary mode
---
...
- Never stage `*.env`, `*.env.*`, `id_rsa`, `id_ed25519`, `*.pem`, `*.key`, or anything in `.gitignore` unless the user explicitly says so.
...
Write a conventional commit message that explains WHY, not just what. Subject ≤ 72 chars, imperative mood. Body explains the user-visible or operational change and any non-obvious trade-offs.

...
> [!IMPORTANT]
> **Source of truth для всех управляемых конфигураций — этот GitHub-репозиторий `explesy/dotfiles`.** Старый Google Drive `OpenCode Config` больше не существует и не используется. Для Pi актуальны файлы в `pi/`, для OpenCode — в `opencode/`.

...
раньше Homebrew в `$PATH`, поэтому новые shell-сессии используют launchers
автоматически.

- `pi` — обычный защищённый режим: permission system спрашивает подтверждение
  для действий, которые не входят в разрешённую политику.
- `pi-trusted` — явный доверенный режим для знакомого локального репозитория:
  запускает тот же базовый Pi, но без permission system и, следовательно, без
  диалогов подтверждения. Агент сможет выполнить любое доступное ему действие,
  включая изменения файлов, удаление, внешние запросы и `git push`; используйте
  его только когда это приемлемо.

Оба режима запускают recap и workflow, но не загружают Herdr-инструменты.
...
(`sudo`, `rm`, destructive git, process kills), `git push`, `node -e` и
незнакомые shell-команды по-прежнему требуют подтверждения. В цепочке каждая
команда проверяется отдельно: разрешённый `cd` не отменяет запрос на
подтверждение для следующего опасного шага. Если такой контроль мешает в
знакомом локальном проекте, используйте явно выбранный `pi-trusted`.

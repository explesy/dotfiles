...
Во время такого запуска `/build` временно исключает только инструмент
Herdsman `agent`: его корневая JSON Schema использует `anyOf`, а некоторые
маршруты Console Go отклоняют её с `400 Invalid schema for function 'agent'`.
Обычные инструменты Pi, включая чтение, поиск, `bash` и редактирование,
остаются доступными. После ответа или закрытия сессии исходный набор
инструментов восстанавливается. Для задач, которым действительно требуется
делегирование Herdsman, отправляйте обычный запрос без `/build`.
